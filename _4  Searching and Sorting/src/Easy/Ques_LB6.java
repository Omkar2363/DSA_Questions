...
public class Ques_LB6 {
    ...
    /*      This problem is mainly an extension of Search an element in an array where difference
            between adjacent elements is 1.
            A Simple Approach is to traverse the given array one by one and compare every element
            with the given element ‘x’. If matches, then return index.
            ...
            find the difference between the current array element and x.
            Let this difference be ‘diff’. From the given property of the array, we always know that x must
            be at least ‘diff/k’ away, so instead of searching one by one, we jump ‘diff/k’.
    * */
    // Java program to search an element in an array where difference between adjacent elements is at-most k
    class GFG {
        ...
        static int search(int arr[], int n, int x, int k)
        {
            ...
            while (i < n) {
                ...
                // We use max here to make sure that ith moves at-least one step ahead.
